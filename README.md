# Robots Meet Arts – Ressources en français

**Quand la pensée computationnelle rencontrent les humanités**

Ce dépôt contient l'ensemble des ressources en français du projet Erasmus+ Robots Meet Arts.

---

## À propos du projet

Robots Meet Arts propose d'intégrer des pratiques de pensée computationnelles dans les enseignements relevant des humanités, à destination des enseignants du primaire. Les activités invitent les élèves à imaginer, créer et s'exprimer : les robots deviennent personnages de récits, danseurs de chorégraphies, explorateurs de civilisations anciennes, messagers entre les langues.

---

## Contenu

**[Modules de formation](1.Modules_Formation/)**
Le parcours complet de 6 modules destiné aux enseignants du primaire, couvrant les approches pédagogiques, les compétences techniques et les pratiques inclusives.

**[Fiches pédagogiques](2.Plans_de_Cours_FR/)**
29 activités clé en main : 8 séances d'introduction et 21 séances disciplinaires réparties dans 7 disciplines relevant des humanités.

**[Mise en œuvre inclusive](3.Inclusion/)**
Recommandations pour adapter les séances afin que tous les élèves puissent y participer pleinement — en tenant compte du genre, des différences d'apprentissage et de la diversité culturelle.

**[Lettres d'information](4.Bulletins_Info/)**
Actualités du projet, nouvelles des partenaires et supports de diffusion.

**[Bonnes pratiques](5.Bonnes_Pratiques/)**
Rapport de bonnes pratiques basé sur l'expérience et les résultats du projet.

**[Livre de cours complet](RMA_LessonBook_FR.pdf)**
L'intégralité du livre de cours regroupant tous les contenus de formation en un seul document.

---

## Autres langues

- English: [rma-english](https://github.com/Robots-Meet-Arts/rma-english)
- Dutch (Belgium): [rma-dutch](https://github.com/Robots-Meet-Arts/rma-dutch)
- Ελληνικά: [rma-ellinika](https://github.com/Robots-Meet-Arts/rma-ellinika)
- Català: [rma-catala](https://github.com/Robots-Meet-Arts/rma-catala)

---

## Partenaires du projet

- L.A.B. – Laboratoire d'Aix-périmentation et de Bidouille (France)
- Universitat Rovira i Virgili (Espagne)
- UCLL (Belgique)
- STIMMULI for Social Change (Grèce)
- DSPI (Grèce)
- Maria Fortuny (Espagne)
- KS Leuven (Belgique)

---

## Licence

Ce travail est mis à disposition selon les termes de la licence Creative Commons Attribution – Partage dans les Mêmes Conditions 4.0 International (CC BY-SA 4.0).

---

## Financement

Cofinancé par l'Union européenne. Les points de vue et opinions exprimés n'engagent que leur(s) auteur(s) et ne reflètent pas nécessairement ceux de l'Union européenne ou de l'EACEA.

Numéro de projet : 2023-1-FR01-KA220-SCH-000151881
